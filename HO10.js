...
var passwordCheckOld = function () 
{
   ...
   */
    ...
    if(stringPassword.length < 8) // determining the first checklist , that beingif the password received from the user is less than 8 characters long
    {
        stringOutput = "Error. The password entered has less than 8 characters. Try Again."; // we must send back a message stating that the password received is not long enough
    }
    else 
    {
        for (var i = 0; i < arrayBadPasswords.length; i++) //setting up a loop so the password can be checked with all the passwords on the "no-no" list
        {
            if(stringPassword === arrayBadPasswords[i]) // determining the second checklist, that being whether or not the password received is on the "no-no" list
            {
            stringOutput = "Error. This password is not allowed. Try Again. "; // message stating password is on list and therefore is unacceptable
            break;
            }
            else 
            {
                stringOutput = "Congratulations. This Password Works!! " //Password works and everyone lives happily ever after.
            }
        }
    ...
    $("output").value = stringOutput; // sending the value back to the HTML through DOM Input
