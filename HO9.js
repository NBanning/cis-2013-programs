...
var generate = function () 
{
	...
	if (isNaN(intCount) || intCount < 2 || intCount > 100) //this first part of the if statement allows for us to vaildate the user input so that the code and function to actually compute a meaningful output
    {
        alert("I'm sorry you entered in an invalid number.");
		$("total_fib").value = "";
    }
	else
	{
	...
	while (intCount > 2)
	{
		k = i + j; // standard fibonacci knowledge, the next number is the sum of the two before it
		stringOutput = stringOutput + k + " ";
		i = j; //the new i value replaces the old value of j
		j = k;  // the new j value replaces the old value of k
		intCount--; // decrement the counter until it reaches > 2
	}
	$("output").value = stringOutput; // send the output back to the html thorugh DOM
	}
